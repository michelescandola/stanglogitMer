...
stanglogitMer = function(dependent,
                         ...
                         covariate,
                         data,
                         cores = 1, chains = 4, warmup = 2000, iter = 4000, seed = NA, control = list(adapt_delta=0.9)){
  ...
  data.list = list(
    ...
    NpredsS = ncol(XFs),
    Nrands  = ncol(XR),
    Ngroup  = length(levels(data[,grouping.string])),
    ...
    grouping= as.numeric(data[,grouping.string]),
